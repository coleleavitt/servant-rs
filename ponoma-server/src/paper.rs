...
impl Default for RiskLimits {
    fn default() -> Self {
        Self { max_order_frac: 0.25, allow_short: false, max_cash_use_frac: 1.0 }
    }
}
...
pub fn risk_check(
    ...
    match action {
        TradeAction::Sell if !limits.allow_short && shares > held_shares + 1e-9 => {
            Err(PaperError::RiskRejected(format!("sell {shares} > held {held_shares} (no shorting)")))
        }
        TradeAction::Buy if notional > limits.max_cash_use_frac * cash + 1e-9 => {
            Err(PaperError::RiskRejected(format!("buy {notional:.2} exceeds available cash {cash:.2}")))
        }
        _ => Ok(()),
    }
}
...
impl Db {
    ...
    pub async fn paper_execute(
        ...
        let act = match action { TradeAction::Buy => "BUY", TradeAction::Sell => "SELL" };
        ...
        sqlx::query("INSERT INTO paper_fill (id, order_id, shares, price) VALUES (?, ?, ?, ?)")
            .bind(Uuid::new_v4().to_string()).bind(&order_id).bind(shares).bind(price)
            .execute(&mut *tx).await?;

        let notional = shares * price;
        let cash_delta = match action { TradeAction::Buy => -notional, TradeAction::Sell => notional };
        ...
        sqlx::query("UPDATE account SET cash = cash + ? WHERE id = ?")
            .bind(cash_delta).bind(account_id).execute(&mut *tx).await?;

        // update or insert the holding (net shares; cost basis = weighted avg on buys)
        let signed = match action { TradeAction::Buy => shares, TradeAction::Sell => -shares };
        let existing = sqlx::query("SELECT id, shares, cost_basis FROM holding WHERE account_id = ? AND ticker = ?")
            .bind(account_id).bind(&ticker).fetch_optional(&mut *tx).await?;
        match existing {
            Some(row) => {
                ...
                if new_shares.abs() < 1e-9 {
                    sqlx::query("DELETE FROM holding WHERE id = ?").bind(&hid).execute(&mut *tx).await?;
                } else {
                    sqlx::query("UPDATE holding SET shares = ?, cost_basis = ? WHERE id = ?")
                        .bind(new_shares).bind(new_cost).bind(&hid).execute(&mut *tx).await?;
                }
            }
            None => {
    ...
    pub async fn transaction_count(&self, account_id: &str) -> Result<i64, sqlx::Error> {
        Ok(sqlx::query("SELECT COUNT(*) AS c FROM transaction_ledger WHERE account_id = ?")
            .bind(account_id).fetch_one(&self.pool).await?.get("c"))
    }
}
...
mod tests {
    ...
    async fn paper_buy_then_sell_updates_ledger_and_holding() {
        ...
        let fill = db.paper_execute(&acct.id, TradeAction::Buy, "VTI", 2.0, 200.0, &limits).await.unwrap();
        ...
        db.paper_execute(&acct.id, TradeAction::Sell, "VTI", 2.0, 250.0, &limits).await.unwrap();
    ...
    async fn risk_gate_rejects_in_engine() {
        let db = bootstrap("sqlite::memory:").await.unwrap();
        let hh = db.households().await.unwrap()[0].id.clone();
        let acct = db.accounts_for_household(&hh).await.unwrap()
            .into_iter().find(|a| a.number == "ROTH-COLE-001").unwrap(); // 1000 cash
        let limits = RiskLimits::default();
        // buy 100 @ 100 = 10000 >> 25% of 1000 → rejected, no ledger entry
        let res = db.paper_execute(&acct.id, TradeAction::Buy, "AAPL", 100.0, 100.0, &limits).await;
